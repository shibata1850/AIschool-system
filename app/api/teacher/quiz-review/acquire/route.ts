...
import {acquireQuizReport,requireReviewer} from "@/lib/quiz-review/service";
...
export async function POST(request:Request){
  try {
    ...
    return Response.json(await acquireQuizReport(actor,input.quizId),{headers});
  }catch(error){return new Response(error instanceof QuizReviewError?error.message:"分析CSVを取得できませんでした",{
