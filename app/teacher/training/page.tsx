...
export default async function TrainingPage() {
  ...
  const links = trainingLinkPolicy(courseId);
  let initial;
  try {
    ...
    initial = stored === null ? { courseId, mode: "legacy" as const, currentDay: null, revision: 0,
      ...
      : parseTrainingSettings(stored, courseId, links);
  } catch { initial = null; }
  return <main style={{ maxWidth: "52rem" }}>
    <Link href="/">ホームにもどる</Link>
    <h1>授業設定</h1>
    {initial ? <TrainingForm initial={initial} links={links} /> : <p role="alert">授業設定を読み込めませんでした。再読み込みしてください。</p>}
  </main>;
}
