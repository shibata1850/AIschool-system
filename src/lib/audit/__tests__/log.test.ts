...
describe("監査ログ（CLAUDE.md 9章）", () => {
  ...
  it("追記のみで順序が保たれる", async () => {
    await recordAudit({ actorRole: "student", action: "create", entity: "submission", entityId: "s1" });
    await recordAudit({ actorRole: "teacher", action: "update", entity: "submission", entityId: "s1" });
    const log = await getAuditLog();
