...
api PUT "/api/v1/courses/${COURSE_ID}" \
  --data-urlencode "course[syllabus_body]=<h2>プロンプト演習デモ（架空）</h2><p>AIに上手に指示（プロンプト）を出す練習をします。毎回、課題のプロンプトを書いて提出し、AIの一次採点と先生のコメントで見直します。</p><ul><li>目標: だれに・どんな長さ・どんな雰囲気を指定できるようになる</li><li>持ち物: 特になし（Quest 3は教室に用意されています）</li></ul>" >/dev/null
echo "[OK] シラバスを設定"
