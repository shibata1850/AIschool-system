...
（JSONが返る）。この設定JSONを手順2でCanvasに貼る（そのままではなく公開鍵を足す）。
...
> ⚠️ **重要**: ltiauthenticator の `/hub/lti13/config` には公開鍵(JWK)が含まれず、
> Canvasは公開鍵を必須にするため、**「URLを入力」方式は使えない**
> （`developer key must have public jwk or public jwk url` で弾かれる）。
> 代わりに、**公開鍵を埋めたJSONを「JSONを貼り付け」方式で貼る**。
> JupyterHubはCanvasへ署名を返さない（ログイン専用）ので、この鍵はダミーでよい。

**2-a. 貼り付け用JSONを生成（Cowork・サーバーで）**
```cmd
bash infra/jupyter/make-canvas-config.sh
REM 出力されたJSON全体を柴田さんへ渡す（Canvasに貼り付ける）
```

**2-b. Canvasでキー作成（柴田さま）**
1. Canvas 管理 → **開発者キー（Developer Keys）** → 「+ 開発者キー」→ **LTIキー**
2. 「メソッド（Method）」で **JSONを貼り付け（Paste JSON）** を選ぶ
3. **JSON欄**に 2-a で生成したJSON全体を貼る
...
> ⚠️ **もう1つのハマりどころ（このCanvasで実際に発生）**: 新しい「Canvas Apps
> （LTIレジストレーション）」方式が有効な環境では、旧「+アプリ → By Client ID」での
> インストールが `このアプリは管理者によってロックされており、インストールできません`
> で拒否される（Canvasの `lock_lti_registrations` 仕様）。
> → 開発者キー一覧の「**Canvas Appsで表示する**」からアプリ管理画面を開き、
> 「**アプリをアンロックする**」を押し、対象アカウントを「**利用可能**」にする。
> その後、下記手順3のようにコースのナビゲーションで表示する。
...
- Canvasキー保存時に `developer key must have public jwk or public jwk url` →
  「URLを入力」方式では通らない。手順2のとおり `make-canvas-config.sh` で公開鍵入りJSONを
  生成し「JSONを貼り付け」方式で登録する
