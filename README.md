...
Next Gen AI School LMSカスタム層（対外呼称「クラウドキャンパス」）。

- 位置づけ: **有限会社ノーティの納品物そのもの**（発注書 SD-2026-0720-001／見積 No.13020653）。
  実装は隘路（ノーティの再委託先）。詳細は `CLAUDE.md` 10章・`docs/位置関係.md`
  （2026-09-02 更新。旧「SOFTDOING内部の参照実装」という位置づけは誤りだった）
- **引き継ぎの入口: `docs/引き継ぎ.md`**（読む順・現在地・作業の作法）
- 仕様: `docs/要件定義書.md`（正本）、`docs/画面仕様書.md`、`docs/テスト計画書.md`
- 未決事項・要望: `docs/未決事項.md` / `docs/要望リスト.md`
- 姉妹システム「eラーニングシステム」との境界: `CLAUDE.md` 13章（二重計上の回避）
...
- UIテキストはすべて日本語（NUC＋モバイルモニター/Quest 3/NearHubの3条件表示制約に従う）
